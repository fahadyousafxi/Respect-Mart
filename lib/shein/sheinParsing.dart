...
class SheinDataParsing {
  ...
  Future<bool> viewProduct(String url) async {
    ...
    try {
      ...
      menu = await http.get(url.replaceAll("https://m", "https://www"));
      this.url = url;
      print('response is: ${url.replaceAll("https://m", "https://www")}');
...
//
//
      var im = document.getElementsByClassName(
          "swiper-slide goodsd-img j-push-history-hash swiper-slide-active");

      //print('Image is: ${im[1].attributes['src']}');
      if (im.length > 0) {
        image = im[0].attributes["data-img"];
        print('Srcs is: $image');
      }
      //For title
      var titleTag = document.head.getElementsByTagName("goodsd-name");
      if (titleTag.length > 0) {
        title = titleTag[0].text.trim();
        print("Title: "+title);
      }*/

      var pr = document.getElementsByClassName("dt-description-kv");
    ...
    /*  var cl = document.getElementsByClassName("color-block");
      if(cl.length>0){
        color = cl[0].text;
        print("Color: "+color);
      ...
      var sz = document.getElementsByClassName("size-active");
