...
class SheinWebView extends StatefulWidget {
  ...
  SheinWebView(this.url);

  @override
  SheinWebViewState createState() {
    try {
      return new SheinWebViewState(this.url);
    } catch (Exception) {
      print(Exception);
    }
  }
...
class SheinWebViewState extends State<SheinWebView> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget webScreen = new Scaffold(
              ...
              onTap: () {
                ...
                _controller.future.then((test) {
                  test.currentUrl().then((value) {
                    ...
                    print("\n\n\n\n\n");
                    temporyurl = value;
                    if (temporyurl.contains("-p-") ) {
                      MyApp.showProgressDialog(context, "Adding to favourite");

                      MyApp.objSheinPrsing
                          .viewProduct(temporyurl)
                          .then((success) {
                        if (MyApp.objSheinPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check =
                              MyApp.objSheinPrsing.addTofav(temporyurl);
                          String desc = "";
                          String title = "";
      ...
      body: WebView(
        ...
        //  webViewController.evaluateJavascript("document.querySelector('meta[name=\"viewport\"]').setAttribute('content', 'width=1024px, initial-scale=' + (document.documentElement.clientWidth / 1024));");
        ...
        //      _controller.future.then((value) => value.evaluateJavascript("document.querySelector('meta[name=\"viewport\"]').setAttribute('content', 'width=1024px, initial-scale=' + (document.documentElement.clientWidth / 1024));"));
                    ...
                    if (check) {
                      ...
                      MyApp.objSheinPrsing
                        ...
                        if (MyApp.objSheinPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check =
                              MyApp.objSheinPrsing.addTocart(temporyurl);
                          String desc = "";
                          String title = "";
                          AlertType alertType = AlertType.success;
