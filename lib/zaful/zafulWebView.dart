...
class ZafulWebView extends StatefulWidget {
  String url = "";
  bool check = false;

  ZafulWebView(this.url);

  @override
  ZafulWebViewState createState() {
    try {
      return new ZafulWebViewState(this.url);
    } catch (Exception) {
      print(Exception);
    }
  }
...
class ZafulWebViewState extends State<ZafulWebView> {
  ...
  void initState() {
    ...
    if (this.url.contains("-puid_")) {
      _showAddToCartButton(true);
      temporyurl = url;
      MyApp.url = url;
    } else {
      MyApp.objZafulPrsing.reset();
              ...
              onTap: () {
                ...
                _controller.future.then((test) {
                  test.currentUrl().then((value) {
                    ...
                    print("\n\n\n\n\n");
                    temporyurl = value;
                    if (temporyurl.contains("-puid_") ) {
                      MyApp.showProgressDialog(context, "Adding to favourite");

                      MyApp.objZafulPrsing
                          .viewProduct(temporyurl)
                          .then((success) {
                        if (MyApp.objZafulPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check =
                              MyApp.objZafulPrsing.addTofav(temporyurl);
                          String desc = "";
                          String title = "";
                    ...
                    if (check) {
                      ...
                      MyApp.objZafulPrsing
                        ...
                        if (MyApp.objZafulPrsing.parsedSuccess && success) {
                          print("Done Scrapping");
                          int check =
                              MyApp.objZafulPrsing.addTocart(temporyurl);
                          String desc = "";
                          String title = "";
                          AlertType alertType = AlertType.success;
