...
class FashionNovaWebView extends StatefulWidget {

  FashionNovaWebView(this.url);
  bool check = false;
  String url = "";


  @override
  FashionNovaWebViewState createState() {
    try {
      return new FashionNovaWebViewState(this.url);
    } catch (Exception) {
      print(Exception);
    }
  }
...
class FashionNovaWebViewState extends State<FashionNovaWebView> {

  FashionNovaWebViewState(this.url);
...
Stores store = new Stores();
  ...
  void initState() {
    ...
    if (this.url.contains("/products/")) {
      _showAddToCartButton(true);
      temporaryUrl = url;
      MyApp.url = url;
    } else {
      MyApp.objFashionNovaPrsing.reset();
              ...
              onTap: () {
                ...
                _controller.future.then((test) {
                  test.currentUrl().then((value) {
                    ...
                    print("\n\n\n\n\n");
                    temporaryUrl = value;
                    if (temporaryUrl.contains("/item") ||
                        temporaryUrl.contains("/d/")) {
                      MyApp.showProgressDialog(context, "Adding to favourite");

                      MyApp.objFashionNovaPrsing
                          .viewProduct(temporaryUrl)
                          .then((success) {
                        if (MyApp.objFashionNovaPrsing.parsedSuccess &&
                            success) {
                          print("Done Scrapping");
                          int check =
                              MyApp.objFashionNovaPrsing.addToFav(temporaryUrl);
                          String desc = "";
                          String title = "";
                    ...
                    if (check) {
                      ...
                      MyApp.objFashionNovaPrsing
                        ...
                        if (MyApp.objFashionNovaPrsing.parsedSuccess &&
                          ...
                          print("Done Scrapping");
                          int check =
                              MyApp.objFashionNovaPrsing.addToCart(temporaryUrl);
                          String desc = "";
                          String title = "";
                          AlertType alertType = AlertType.success;
